...
wait_for_ias () {
  ...
  while [ "$timer" -gt 0 ] && [[ $progress == *"in progress"* ]]; do
      printf "Waiting until Identity instance$CYAN java-ams-ias$RS is created. %s seconds left\n" "$timer"
      progress=$(cf service java-ams-ias | grep "status:")
      printf "%s\n" "$progress"
      (( timer=timer-5 ))
      sleep 5
      if [[ timer -eq 0 ]];then
...
printf "Creating identity service$CYAN java-ams-ias$RS\n"
progress=$(cf cs identity application java-ams-ias -c "$(sed "s/((ID))/$ID/;s/((LANDSCAPE_APPS_DOMAIN))/$DOMAIN/;s/((MT))/$MT/g" ./ias-config.json)")
# cf service creation status options: "create failed"| "already exists" | "in progress" | "succeeded"
if [[ $progress != *"already exists"* ]]; then
  wait_for_ias
  printf "Identity service$CYAN java-ams-ias$RS was created successfully\n"
elif [[ $progress == *"FAILED"* ]]; then
  printf $progress
else
  printf "Identity service$CYAN java-ams-ias$RS already exists\n"
fi


# Create SMS service instance if IAS was created successfully
if [[ $progress ==  *"succeeded"* || $progress ==  *"already exists"* ]]; then
  printf "Creating subscription manager service$CYAN java-ams-sms$RS\n"
  sms_progress=$(cf cs subscription-manager provider java-ams-sms -c "$(sed "s/((ID))/$ID/;s/((LANDSCAPE_APPS_DOMAIN))/$DOMAIN/g" ./sms-config.json)")
  if [[ $sms_progress != *"already exists"* ]]; then
      sms_progress=$(cf service java-ams-sms | grep "status:")
      ...
      if [[ $sms_progress == *"succeeded"*  ]]; then
          printf "Subscription manager service$CYAN java-ams-sms$RS created successfully\n"
      fi
  else
      printf "Subscription manager service$CYAN java-ams-sms$RS already exists\n"
  fi
elif [[ $progress == *"failed"* ]]; then
  printf "Couldn't create Identity service$CYAN java-ams-ias$RS: %s" "$progress"
fi
