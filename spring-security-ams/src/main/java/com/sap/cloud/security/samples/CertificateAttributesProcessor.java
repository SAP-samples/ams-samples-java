...
public class CertificateAttributesProcessor implements AttributesProcessor {
    ...
    public void processAttributes(Principal principal) {
        if (principal.getId() == null) {
            HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
                    .getRequest();
            List planPolicies = determinePoliciesForServicePlan(request.getHeader(FWD_CLIENT_CERT_HEADER));
            if (planPolicies.isEmpty()) {
                ...
                throw new InsufficientAuthenticationException("Can't find policies for calling client."); // 401
            }
            principal.getAttributes().setPolicies(planPolicies); // sets 0..n policies [AND the default policies]
        }
    }
    ...
    private List determinePoliciesForServicePlan(String clientCertHeader) {
        ...
        if (clientCert != null) {
            ...
            if (subjectDNs.containsKey("OU") && subjectDNs.get("OU").contains("SAP Cloud Platform Clients")) {
