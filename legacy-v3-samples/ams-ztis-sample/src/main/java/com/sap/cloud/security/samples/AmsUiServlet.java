...
public class AmsUiServlet extends HttpServlet {
  ...
  private static final OAuth2ServiceConfiguration serviceConfig =
      Environments.getCurrent().getIasConfiguration();
  private static final DefaultJsonObject vcapServices =
      new DefaultJsonObject(System.getenv(ServiceConstants.VCAP_SERVICES));
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    String subdomain =
        forwardedHeader.substring(0, forwardedHeader.indexOf(System.getenv("SUBSCRIPTION_SUFFIX")));

    JsonObject serviceJsonObject = vcapServices.getJsonObjects("subscription-manager").get(0);
    Map<String, String> credentialsMap =
        serviceJsonObject.getJsonObject("credentials").getKeyValueMap();
    String uaaDomain = credentialsMap.get("uaadomain");

    String url =
        String.format(
            "https://api.%s/sap/rest/tenantLoginInfo/%s?iasClient=%s",
            uaaDomain, subdomain, serviceConfig.getClientId());
    LOGGER.debug("tenant info url = {}", url);
    HttpRequest httpRequest = HttpRequest.newBuilder(URI.create(url)).GET().build();

    try {
      // Send the request and retrieve the response
      HttpResponse<String> xsuaaResponse =
          httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString());

      if (xsuaaResponse.statusCode() != 200) {
        LOGGER.error(
            String.format(
                "unexpected response status from tenant info endpoint: %s",
                xsuaaResponse.statusCode()));
        response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
        return;
      }

      JSONObject jsonData = new JSONObject(xsuaaResponse.body());
      if (!Objects.equals(jsonData.getString("status"), "ACTIVE")
          || !Objects.equals(jsonData.getString("authentication"), "ias")) {
        LOGGER.error(
            String.format(
                "unexpected response from tenant info endpoint: %s", xsuaaResponse.body()));
        response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
        return;
      }
      ...
      String iasUrl =
          jsonData
              ...
              .substring(
                  0, jsonData.getString("authorization_endpoint").indexOf("/", 8)); // skip https://
