...
public class SalesOrderController {
  ...
  /**
   * An endpoint showing how to use Spring method security. Only if the request principal has the
   * given privilege he/she is allowed to access the method. Otherwise a 403 error will be returned.
   */
  ...
  @PreAuthorize(
      "forResourceAction('salesOrders', 'read', 'CountryCode:string=' + #countryCode, 'salesOrder.type:number=' + #type)")
  @GetMapping(value = "/readByCountryAndType/{countryCode}/{type}")
  public String readSelectedSalesOrder(
      @PathVariable String countryCode, @PathVariable String type) {
    return "Read-protected SalesOrder with attributes countryCode = "
        + countryCode
        + " and type "
        + type
        + " accessed!";
