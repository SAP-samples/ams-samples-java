...
class JavaServletIntegrationTest {
  ...
  static SecurityTestExtension extension =
      SecurityTestExtension.forService(IAS)
  ...
  public static void assumePolicyEngineIsReachable() {
      ...
      boolean reachable = pdp.isReachable();
      System.out.println("PDP erreichbar: " + reachable);
      assumeTrue(reachable, "Policy Decision Point ist nicht erreichbar. Bitte überprüfe die Konfiguration.");
  }
  ...
  @Test
  void requestWithToken_missingPermission_unauthorized(SecurityTestContext context)
      throws IOException {
    String jwt =
        context
            .getPreconfiguredJwtGenerator()
            .withClaimValues("test_policies", "test.notExisting")
            .createToken()
            .getTokenValue();
  ...
  void requestWithToken_withPermission_ok(SecurityTestContext context) throws IOException {
    String jwt =
        context
            .getPreconfiguredJwtGenerator()
            .withClaimValues("test_policies", "ams.readAllSalesOrders")
            .createToken()
            .getTokenValue();
    ...
    try (CloseableHttpResponse response = HttpClients.createDefault().execute(request)) {
      ...
      assertEquals(
          "Read-protected method called!", EntityUtils.toString(response.getEntity(), "UTF-8"));
    }
  }
  ...
  void requestWithToken_withCustomPermission(SecurityTestContext context) throws IOException {
    String jwt =
        context
            .getPreconfiguredJwtGenerator()
            .withClaimValues("test_policies", "ams.readAllOwnItems")
            .createToken()
            .getTokenValue();
    ...
    try (CloseableHttpResponse response = HttpClients.createDefault().execute(request)) {
      ...
      assertThat(
          ...
          containsString(
              "Retrieved access constraint: eq($app.author.createdBy, \"the-user-id\")"));
    }

    jwt =
        context
            .getPreconfiguredJwtGenerator()
            .withClaimValues("test_policies", "ams.readAllOwnItems_new")
            .createToken()
            .getTokenValue();
    ...
    try (CloseableHttpResponse response = HttpClients.createDefault().execute(request)) {
      ...
      assertThat(responseString, containsString("Retrieved access constraint: or(\n"));
      assertThat(responseString, containsString("  eq($app.author.createdBy, \"the-user-id\")\n"));
      assertThat(responseString, containsString("  eq($app.author.updatedBy, \"the-user-id\")\n"));
  ...
  void request_withBotUserToken_200() throws Exception {
    ...
    try (CloseableHttpResponse response = HttpClients.createDefault().execute(request)) {
      ...
      assertThat(
          EntityUtils.toString(response.getEntity(), "UTF-8"),
          containsString(
              "Read-protected method called. Retrieved access constraint: eq($app.author.createdBy, \"d9403e85-2029-46f1-9c09-ee32e881c081\")"));
    }
  }
