...
@WebFilter(urlPatterns = "/api/*", filterName = "iasTokenValidator") // filter for any endpoint
public class IasSecurityFilter implements Filter {
  ...
  public static void sendUnauthorizedResponse(
      ServletResponse response, Attributes attributes, String message) {
    if (response instanceof HttpServletResponse httpServletResponse) {
      try {
        String user =
            Objects.nonNull(SecurityContext.getAccessToken())
                ? SecurityContext.getToken().getClaimAsString(USER_NAME)
                : "<Unknown>";
        LOGGER.error("User {} is unauthorized with {}. Message: '{}'", user, attributes, message);
        httpServletResponse.sendError(
            HttpServletResponse.SC_FORBIDDEN, "User " + user + " is unauthorized."); // 403
      } catch (IOException e) {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try {
      TokenAuthenticationResult authenticationResult =
