package com.sap.cloud.security.samples.ztis.mtls;
...
public class X509SourceSingletonWrapper {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(X509SourceSingletonWrapper.class);

  private static final String DEFAULT_SPIFFE_ENDPOINT_SOCKET =
      "unix:///tmp/spire-agent/public/api.sock";

  private static X509Source x509Source;

  private static X509SourceSingletonWrapper x509SourceSingletonWrapper =
      new X509SourceSingletonWrapper();
    ...
    synchronized (X509SourceSingletonWrapper.class) {
      ...
      DefaultX509Source.X509SourceOptions x509SourceOptions =
          DefaultX509Source.X509SourceOptions.builder()
              .spiffeSocketPath(spiffeEndpointSocket)
              .build();
  ...
  public static boolean isSvidAvailable() {
    try {
      ...
      LOGGER.warn(
          "No Svid available (No Svid has been retrieved via Workload API). Exception: {}",
          e.getMessage());
    }
    return false;
  }
